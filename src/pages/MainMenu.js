function MainMenu({onMouseLeave}){
    return(
      <div id='MainMenu' className={`headerMegaMenu_mega_menu__IqR7o  `} onMouseLeave={onMouseLeave}>
      <div
        className={`headerMegaMenu_mega_menu_main__zLCmF `}
        data-lenis-prevent='true'
        style={{
          opacity: 1,
          transform: 'translate3d(0px, 0px, 0px)',
          willChange: 'transform',
          transition: '5',
          zIndex:99
        }}
      >
        <div className='headerMegaMenu_mega_menu_main_scroll__1abSn'>
          <div className='container'>
            <span
              className='headerMegaMenu_gradient_item_01__7FZ1y'
              style={{
                background: 'linear-gradient(154deg, rgb(0, 0, 0) 50%, rgba(0, 32, 137, 0.9) 75.71%, rgb(0, 69, 255) 72.36%, rgb(0, 196, 255) 100%)',
                transform: 'translate3d(0px, 0px, 0px) rotate(-40deg) skew(2.893deg, 0deg) scale(0.97965, 0.96973)',
                opacity: 0.4,
              }}
            ></span>
            <div className='undefined row'>
              <div
                className='headerMegaMenu_content_wrapper__q7HPU col-lg-3'
                style={{
                  opacity: 1,
                  transform: 'translate3d(0px, 0px, 0px)',
                }}
              >
                <h2 className='headerMegaMenu_title__0sLjU fw-light mb-0 text-start'>Building Strong Capabilities to Empower Your Brand</h2>
                <a aria-label='link to home' className='btn btn-secondary light headerMegaMenu_main_link__PjzV9' href='/'>
                  <span>Go to overview</span>
                  <span className='headerMegaMenu_icon__rqItq'>
                    <svg className='' width='12' height='12' viewBox='0 0 1538 1024' xmlns='http://www.w3.org/2000/svg' style={{ display: 'inline-block', verticalAlign: 'middle' }}>
                      <path d='M1006.294 1024l-67.791-70.432 458.589-441.568-458.687-441.568 67.791-70.432 531.956 512z'></path>
                      <path d='M1467.329 560.813h-1467.329v-97.822h1467.329z'></path>
                    </svg>
                  </span>
                </a>
              </div>
              <div className='undefined col-lg-9'>
                <div className='headerMegaMenu_menu_links_wrapper__bHf5W'>
                  <div className='row row-cols-3'>
                    <div
                      style={{
                        opacity: 1,
                        transform: 'translate3d(0px, 0px, 0px)',
                      }}
                    >
                      <div className='headerMegaMenu_menu_links__RDsZ_ text-start'>
                        <a className='headerMegaMenu_sub_title__mX5OX ttl-20 fw-medium' href='/'>
                          Branding{' '}
                          <svg className='' width='15' height='15' viewBox='0 0 1538 1024' xmlns='http://www.w3.org/2000/svg' style={{ display: 'inline-block', verticalAlign: 'middle' }}>
                            <path d='M1006.294 1024l-67.791-70.432 458.589-441.568-458.687-441.568 67.791-70.432 531.956 512z'></path>
                            <path d='M1467.329 560.813h-1467.329v-97.822h1467.329z'></path>
                          </svg>
                        </a>
                        <ul>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Brand Consulting
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Logo Design
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Industrial / Product Design
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Graphic Design
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              2D / 3D Visualisation
                            </a>
                          </li>
                        </ul>
                      </div>
                      <div className='headerMegaMenu_menu_links__RDsZ_ text-start'>
                        <a className='headerMegaMenu_sub_title__mX5OX ttl-20 fw-medium' href='/'>
                          Experience Design{' '}
                          <svg className='' width='15' height='15' viewBox='0 0 1538 1024' xmlns='http://www.w3.org/2000/svg' style={{ display: 'inline-block', verticalAlign: 'middle' }}>
                            <path d='M1006.294 1024l-67.791-70.432 458.589-441.568-458.687-441.568 67.791-70.432 531.956 512z'></path>
                            <path d='M1467.329 560.813h-1467.329v-97.822h1467.329z'></path>
                          </svg>
                        </a>
                        <ul>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              UI/UX Design
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Web Experience
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Mobile Experience
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Commerce Experience
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Human Machine Interface
                            </a>
                          </li>
                        </ul>
                      </div>
                    </div>
                    <div
                      style={{
                        opacity: 1,
                        transform: 'translate3d(0px, 0px, 0px)',
                      }}
                    >
                      <div className='headerMegaMenu_menu_links__RDsZ_ text-start'>
                        <a className='headerMegaMenu_sub_title__mX5OX ttl-20 fw-medium' href='/'>
                          Technology{' '}
                          <svg className='' width='15' height='15' viewBox='0 0 1538 1024' xmlns='http://www.w3.org/2000/svg' style={{ display: 'inline-block', verticalAlign: 'middle' }}>
                            <path d='M1006.294 1024l-67.791-70.432 458.589-441.568-458.687-441.568 67.791-70.432 531.956 512z'></path>
                            <path d='M1467.329 560.813h-1467.329v-97.822h1467.329z'></path>
                          </svg>
                        </a>
                        <ul>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              AI & Machine Learning
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              DevOps Consulting
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Data & Analytics
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Web Development
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Mobile App Development
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              E-commerce
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Quality Assurance & Testing
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Cloud Services
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Cyber Security
                            </a>
                          </li>
                        </ul>
                      </div>
                    </div>
                    <div
                      style={{
                        opacity: 1,
                        transform: 'translate3d(0px, 0px, 0px)',
                      }}
                    >
                      <div className='headerMegaMenu_menu_links__RDsZ_ text-start'>
                        <a className='headerMegaMenu_sub_title__mX5OX ttl-20 fw-medium' href='/'>
                          Digital Marketing{' '}
                          <svg className='' width='15' height='15' viewBox='0 0 1538 1024' xmlns='http://www.w3.org/2000/svg' style={{ display: 'inline-block', verticalAlign: 'middle' }}>
                            <path d='M1006.294 1024l-67.791-70.432 458.589-441.568-458.687-441.568 67.791-70.432 531.956 512z'></path>
                            <path d='M1467.329 560.813h-1467.329v-97.822h1467.329z'></path>
                          </svg>
                        </a>
                        <ul>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Search Engine Optimisation
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Social Media Management
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Performance Marketing
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Content Marketing
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Marketing Automation
                            </a>
                          </li>
                          <li>
                            <a className='headerMegaMenu_link__RWzsb' href='/'>
                              Analytics
                            </a>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    )
}
export default MainMenu
